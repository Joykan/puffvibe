    ...
    <div className="container mx-auto px-4 py-8">
        ...
        {apiStatus === 'connected' && (
          <div className="bg-green-50 border border-green-200 rounded-lg p-4">
            <div className="flex items-center">
              ...
              <div>
                <p className="text-green-800 font-medium">Connected to PuffVibe Server</p>
                <p className="text-green-700 text-sm">Real-time order processing enabled</p>
              </div>
            </div>
          </div>
        )}
      ...
      <div className="bg-green-50 border border-green-200 rounded-lg p-4 mb-6">
        <div className="flex items-center">
          ...
          <div>
            <h3 className="font-semibold text-green-800">Ultra-Fast Maseno Delivery</h3>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {products.map(product => (
          <div key={product._id} className="bg-white rounded-lg shadow-lg border p-6">
            ...
            <p className="text-green-600 mb-4">In Stock: {product.stock.current} units</p>
            
            <div className="space-y-4">
              {product.pricingTiers.map((tier, index) => (
                <div key={index} className="border rounded-lg p-4 hover:border-purple-500 transition">
                  <div className="flex justify-between items-center mb-2">
                    <h3 className="font-semibold text-gray-800">{tier.name}</h3>
                    <span className="text-lg font-bold text-purple-600">KSh {tier.price}</span>
                  </div>
      ...
      {cart.length > 0 && (
        <div className="fixed bottom-4 right-4 bg-white p-6 rounded-lg shadow-xl border max-w-sm z-50">
          ...
          <button 
            onClick={proceedToCheckout}
            className="w-full bg-green-600 text-white py-3 rounded-lg mt-4 hover:bg-green-700 transition font-semibold"
          >
            Checkout Now ›
          </button>
        </div>
